...
#define CAPTURE_INPUT "360AA.mp4"     // THETA S の Equirectangular 画像
#define LOADIMAGE                       //load extern image
...
int main()
{
	...
	camera.start();
#else
	Mat inputImg = imread("1.jpg");
	int imgWidth = inputImg.cols;
	int imgHeight = inputImg.rows;
...
#else
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, imgWidth, imgHeight, 0, GL_BGR, GL_UNSIGNED_BYTE, NULL);
#endif // LOADIMAGE
	...
	while (!window.shouldClose())
	{
		...
		camera.transmit();
#else
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, imgWidth, imgHeight, 0, GL_BGR, GL_UNSIGNED_BYTE, inputImg.data);
#endif // LOADIMAGE
